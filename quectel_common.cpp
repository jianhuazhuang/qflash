...
proto_name_t proto_list[] = 
{
    PROTO_NAME(PROTO_FASTBOOT),
    PROTO_NAME(PROTO_STREAM),
    PROTO_NAME(PROTO_FASTBOOT_AT),
    PROTO_NAME(PROTO_FIREHOSE),
};
...
void upgrade_process(int writesize,int size,int clear)
{
    ...
    if(progress == 100)
    {
        printf( "%s progress : %d%% finished \n", get_time(), progress);        
        //fflush(stdout);
    }
    else
    {
        printf( "%s process: %d%% finished\r", get_time(), progress/*,  transfer_statistics::getInstance()->get_percent()*/);
        //fflush(stdout);
    }
