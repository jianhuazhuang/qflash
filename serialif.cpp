...
static void handle_sahara_protocol(unsigned char *rx_buff, target_current_state* state_)
{
	...
	switch(id)
	{
		case SAHARA_HELLO_ID:
			{
				dbg_time("Sahara Command = SAHARA_HELLO_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_READ_DATA_ID:
			{
				dbg_time("Sahara Command = SAHARA_READ_DATA_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_DONE_ID:
			{
		...
		case SAHARA_END_IMAGE_TX_ID:
			{
				...
				dbg_time("Sahara Command = SAHARA_END_IMAGE_TX_ID\n");
				dbg_time("end_of_image_transfer_packet->Command = %d\n", ptr->Command);
				dbg_time("end_of_image_transfer_packet->Length = %d\n", ptr->Length);
				dbg_time("end_of_image_transfer_packet->ImageID = %08x\n", ptr->ImageID);
				dbg_time("end_of_image_transfer_packet->Status=%08x\n", ptr->Status);
				*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_RESET_ID:
			{
				dbg_time("Sahara Command = SAHARA_RESET_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_MEMORY_DEBUG_ID:
			{
				dbg_time("Sahara Command = SAHARA_MEMORY_DEBUG_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_MEMORY_READ_ID:
			{
				dbg_time("Sahara Command = SAHARA_MEMORY_READ_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_CMD_READY_ID:
			{
				dbg_time("Sahara Command = SAHARA_CMD_READY_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_CMD_SWITCH_MODE_ID:
			{
				dbg_time("Sahara Command = SAHARA_CMD_SWITCH_MODE_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_CMD_EXEC_ID:
			{
				dbg_time("Sahara Command = SAHARA_CMD_EXEC_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_CMD_EXEC_DATA_ID:
			{
				dbg_time("Sahara Command = SAHARA_CMD_EXEC_DATA_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_64_BITS_MEMORY_DEBUG_ID:
			{
				dbg_time("Sahara Command = SAHARA_64_BITS_MEMORY_DEBUG_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_64_BITS_MEMORY_READ_ID:
			{
				dbg_time("Sahara Command = SAHARA_64_BITS_MEMORY_READ_ID\n");*state_ = STATE_SAHARA_MODE;
			}
			break;
		case SAHARA_64_BITS_READ_DATA_ID:
			{
				dbg_time("Sahara Command = SAHARA_64_BITS_READ_DATA_ID\n");
				*state_ = STATE_SAHARA_MODE;
			}
			break;
		default:
			{
				dbg_time("Sahara Command = Unknown\n");
...
int SendHelloPacketTest(int emergency_mode)
{
	...
	//module in normal state and 9x07 9x40 platform, send 1024 negotiatory bytes
		...
		{
			dbg_time("Send Zero packet error\n");
			return 0;
		}
		sleep(3);
	}
